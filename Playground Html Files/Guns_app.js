  ...
  document.addEventListener('mouseup', () => {
    ...
    loadWeapons() {
        const weaponData = [
            { 
                ...
                baseScale: 1915,
                path: new THREE.CatmullRomCurve3([
                    new THREE.Vector3(-6, -2, 4), new THREE.Vector3(-3, 0, 7),
                    new THREE.Vector3(0, 0.5, 3), new THREE.Vector3(2, 0, -1),
                    new THREE.Vector3(0, 0, 0)
                ])
            },
            { 
            ...
            { 
                ...
                baseScale: 550,
                path: new THREE.CatmullRomCurve3([
                    new THREE.Vector3(-5, -3, -2), new THREE.Vector3(-2, -3, 0),
                    new THREE.Vector3(0, -3.5, 1), new THREE.Vector3(2, -3, 0),
                    new THREE.Vector3(5, -3, -2)
                ])
            }
        ];
            ...
            this.loader.load(data.url, (gltf) => {
                ...
                model.traverse((child) => {
                    if (child.isMesh) {
                        child.castShadow = true;
                        child.receiveShadow = true;
                        child.material.side = THREE.DoubleSide; // More performant
                    }
                });
    ...
    animate() {
        ...
        this.weapons.forEach((weapon) => {
            ...
            weapon.mesh.rotation.y += this.mouse.x * 0.5 * (weapon.pathProgress - prevProgress);
            weapon.mesh.rotation.x -= this.mouse.y * 0.5 * (weapon.pathProgress - prevProgress);
            
            // Fade in at the start, fade out at the end of the path
            const visibility = Math.sin(weapon.pathProgress * Math.PI);
            
            const scale = (weapon.baseScale / 50) * visibility;
            weapon.mesh.scale.set(scale, scale, scale);
